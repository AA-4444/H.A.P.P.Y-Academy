  ...
  const points = useMemo(
	() =>
	  [
		...
		[-79.3832, 43.6532],
		[-122.3321, 47.6062],
	  ].map((c, idx) => ({ id: `p-${idx}`, coordinates: c as [number, number] })),
	[]
  );
	...
	>
	  ...
	  <div className="absolute inset-0 z-0 hidden sm:block">
		...
		<div
		  ...
		  style={{
			background:
			  "linear-gradient(to bottom, rgba(243,238,230,0.92) 0%, rgba(243,238,230,0.72) 42%, rgba(243,238,230,0.55) 100%)",
		  }}
		/>
	  </div>

	  {/* ✅ MOBILE: карта НЕ фоном, а отдельным блоком под текстом (полностью видна) */}
	  <div className="relative z-10 sm:hidden">
		...
		{/* ✅ блок карты на телефоне: фиксированная “красивая” высота, весь мир виден */}
		<div className="mt-6 px-2">
		  <div
			className="w-full"
			style={{
			  height: "min(46svh, 360px)", // ✅ не на всю высоту, но большая
			}}
		  >
			<WorldMapCoverBackground
			  geoUrl={GEO_URL}
			  ...
			  accent={ACCENT}
			  reveal={reveal}
			  mode="contain" // ✅ главное: целиком помещаем карту на мобильном
			/>
		  </div>
		</div>

		{/* небольшой “воздух” перед CTA */}
		<div className="h-6" />
	  </div>
	  ...
	  <div className="relative z-10 hidden sm:block">
		...
		{/* место чтобы был виден фон-карта */}
		<div className="h-[58svh] sm:h-[62svh]" />
	  </div>

	  {/* ✅ CTA: sticky внутри секции (не переезжает на следующую) */}
...
/**
 * mode:
 * - "cover"   => заполняет блок (как фон), может обрезать края (красиво на десктопе)
 * - "contain" => целиком показывает карту (идеально для мобилы)
 */
  ...
  return (
	<div className="w-full h-full">
	  <ComposableMap
		...
		projectionConfig={{
		  // на мобиле можно чуть меньше, чтобы точно помещалось
		  scale: mode === "cover" ? 175 : 150,
		}}
		className="w-full h-full"
		style={{ width: "100%", height: "100%" }}
		preserveAspectRatio={mode === "cover" ? "xMidYMid slice" : "xMidYMid meet"}
	  >
		<ZoomableGroup zoom={1} minZoom={1} maxZoom={1} center={[0, 12]}>
			  ...
			  geographies.map((geo) => (
				<Geography
				  ...
				  style={{
					...
					pressed: { fill: "rgba(0,0,0,0.08)", outline: "none" },
				  }}
				/>
			  ))
			}
		  ...
		  {visible.map((p, idx) => (
			<Marker key={p.id} coordinates={p.coordinates}>
			  <PulsingDot accent={accent} delay={Math.min(0.7, idx * 0.02)} />
			</Marker>
		  ))}
		</ZoomableGroup>
	  </ComposableMap>
